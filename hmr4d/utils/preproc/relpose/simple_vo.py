...
class SimpleVOImages:
    def __init__(self, image_paths, scale=0.5, step=8, method="sift", f_mm=None):
        """
        Args:
            image_paths: list of str, paths to images (ordered)
            scale: float, resize factor for images
            step: int, frame sampling step
            method: str, feature matching method
            f_mm: float or None, focal length in mm
        """
    ...
    def compute(self):
        ...
        frames = [cv2.imread(str(p))[..., ::-1] for p in self.image_paths
                  if p.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp"]]
        frames = [cv2.resize(f, (0, 0), fx=self.scale, fy=self.scale) for f in frames]
        ...
        frames = frames[sample_idxs]
        F, H, W, C = frames.shape
        print(f"[SimpleVOImages] Choosen frames shape: {frames.shape}")

        matcher: Matcher = Matcher(self.method)
        camera_params = CameraParams(W, H, focal_length=focal_length_from_mm(W, H, self.f_mm))
        solver: TwoPairSolver = TwoPairSolver(camera_params, solver="pycolmap")

        T_w2c_list = self.process_images_T_w2c_list_np(frames, matcher, solver)
    ...
    def process_images_T_w2c_list_np(self, frames, matcher: Matcher, solver: TwoPairSolver):
        T_w2c_list = [np.eye(4)]  # cam poses are defined as T_w2c @ p_w = p_c
        prev_frame = frames[0]
        for frame_idx in tqdm(range(1, len(frames))):
            curr_frame = frames[frame_idx]
