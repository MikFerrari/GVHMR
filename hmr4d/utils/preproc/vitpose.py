...
class VitPoseExtractor:
    def __init__(self, tqdm_leave=True):
        ckpt_path = "inputs/checkpoints/vitpose/vitpose-h-multi-coco.pth" #vitpose-<b/l/h>-multi-coco.pth"
        self.pose = build_model("ViTPose_huge_coco_256x192", ckpt_path) #"ViTPose_<base/large/huge>_coco_256x192", ckpt_path)
        self.pose.cuda().eval()
    ...
    @torch.no_grad()
    def extract_frames(self, frames_path, bbx_xys, img_ds=0.5):
        # Get the batch
        if isinstance(frames_path, list):
            imgs, bbx_xys = get_batch(frames_path, bbx_xys, img_ds=img_ds, path_type="image_list")
        else:
            assert isinstance(frames_path, torch.Tensor)
            imgs = frames_path
        ...
        vitpose = torch.cat(vitpose, dim=0).clone()  # (L, 17, 3)
        return vitpose
